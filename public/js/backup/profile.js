	...
	$.ajax({
		...
		success:function(data){
			...
			if(platform==='none')
				...
				check_pay();
			    moneyvalidation();
				$('#creator_money').attr("placeholder", "Enter Values Between 5500 - 1000000");
...
function getaccountregcharge(){//here after click convert you will get This data on form from database
	...
	$.ajax({
		...
		type:"get",
		success:function(data){
			var btc_charge=data.users[0].btc_charge;
			...
			alert('You Must Pay:BTC:'+btc_charge+' On This Address '+internal_wallet+' To Activate This Account');
			$('#pay').text('You Must Pay:BTC:'+btc_charge+' On This Address '+internal_wallet+' To Activate This Account');
		}
...
function getaccountpaydata(){//here after click convert you will get This data on form from database
	...
	$.ajax({
		...
		type:"get",
		success:function(data){
			var btc_childmoney=data.users[0].btc_childmoney;
			var internal_wallet=data.users[0].internal_wallet;
			...
			alert('You Must Pay:BTC:'+btc_childmoney+' On This Address '+internal_wallet+' To Activate This Account');
			console.log(btc_childmoney);
		}
...
"use strict";
	...
	$('#creator_money').keydown(function(){
		var creator_money=$('#creator_money').val();
		if(creator_money>1000000)
			{
				 $('#sbv').hide();	
				$('#reinvvalue').hide();
			}
		else if(creator_money>=5500)
			{
				$('#sbv').show();
				$('#reinvvalue').show();
			}
			...
			var remain=btc_charge-api_money;
			 ...
			 swal({
                ...
                type: "success"
					...
					var notfullpaid=`you have  ${api_money} in your wallet Remaining are ${remain} pay on This ${internal_wallet}`;
			 swal({
...
function check_pay(){//Start Code check_reg_charge This code will be placed on none platform
	...
	if(data)
				{
			var api_money=data.api_money;
			var btc_childmoney=data.btc_childmoney;
			var btc_charge=data.btc_charge;
			...
			var remain=btc_childmoney-invest_money;
			 ...
			 swal({
                ...
                type: "success"
				    ...
				    var internal_wallet=data.internal_wallet;
					var notfullpaid=`you have ${invest_money}  in Your wallet Remaining are ${remain} pay on This ${internal_wallet}`;
			 swal({
                title: "You have not yet Full paid Registration fees",
                text:notfullpaid,
                type: "success"
...
$(document).on("click","#super_child1converted",function(ev){
	...
	//not complete
		$.ajax({
				...
				success:function(data)
				{
			var btc_charge=data.super_child1btcmoney;
...
$(document).on("click","#super_child2converted",function(ev){
	...
	//not complete
		$.ajax({
				...
				success:function(data)
				{
					var btc_charge=data.super_child2btcmoney;
			var internal_wallet=data.super_creatorinternwallet;
...
"use strict";
	...
	$('#supercreator_money').keydown(function(){
		...
		else if(supercreator_money>=9300)
			{
					...
					//////////////////////////////////
						...
						$('.test').hide();
		                 $('#super_payment').show();
							...
							var calcul=req_money-profile_earning2
							var remain=calcul/btc_zar;
							 var internal_wallet=$('#super_creatorinternwallet').val();
					var notfullpaid=`you have  ${profile_earning2} Remaining are BTC:${remain} pay on This ${internal_wallet}  "R9300" This The Minimum Amount To Upgrade`;
			 swal({
...
//            });
	   ...
	   var profile_earning2=$('#profile_earning2').text();
							...
							var remain=remainzar/btc_zar;
							
							var mess=`You have ${profile_earning2} You Must Add ${remain} To ${internal_wallet} for Minimum Creating Account`;
	   ...
	   var profile_earning3=$('#profile_earning2').text();
							...
							var remain2=remainzar2/btc_zar2;
							var mess2=`You have ${profile_earning3} You Must Add ${remain2} To ${internal_wallet2} for Minimum Creating Account`;
...
function super_checkchild1pay()
{
	...
	if(data)
				{
			var api_money=data.api_money;
			var btc_childmoney=data.super_child1btcmoney;
			var btc_charge=data.btc_charge;
			var invest_money=api_money-btc_charge;
			var remain=btc_childmoney-invest_money;
...
function super_checkchild2pay()
{
	...
	if(data)
				{
			var api_money=data.api_money;
			var btc_childmoney=data.super_child2btcmoney;
			var btc_charge=data.btc_charge;
			var invest_money=api_money-btc_charge;
			var remain=btc_childmoney-invest_money;
