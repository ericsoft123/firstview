...
class SbvtriangleController extends Controller
{
	...
	public function trianglesplit()
	{
		if (Auth::check()) {
			...
			$child1_username = Auth::user()->username;
				$triangles=DB::select("select *from triangle_grands where child1_username='$child1_username' and triangle_status='built'");
			foreach($triangles as $triangle)
			{
				 $triangle_parent=$triangle->triangle_parents;
		    $child_number=$triangle->child_number;
		  $child_money=$triangle->child1_money;
			}
			$checktriangles=DB::select("select *from triangle_grands where creator_username='$triangle_parent' and triangle_status='built' and child_number!=$child_number");
			foreach($checktriangles as $checktriangle)
			{
				$child2_username=$checktriangle->child1_username;
			}
			
			////get creator_status
			$creator_checks=DB::select("select*from users where username='$triangle_parent'");
			foreach($creator_checks as $creator_check)
			{
				$creator_status=$creator_check->creator_status;
			}
			...
			$child1=DB::select("select *from users where username='$child1_username'  and platform='child1_invested' ");
			$child2=DB::select("select *from users where username='$child2_username'  and platform='child1_invested' ");
			...
			$creator_values=($total*60)/100;
			 ...
			 $country_managers=$twochild-$creator_values;
				...
				DB::table('users')
					...
					->where([
						'creator_username'=>$triangle_parent,
						'child_number'=>1,
					...
					$checkuser1=DB::select("select *from earnings where username='$triangle_parent'");
			if($checkuser1)
			{
				DB::update("update earnings set sbv=sbv+'$creator_values',total=super+sbv");
			}
			else{
				
				DB::insert("insert into earnings (username,sbv,total) values('$triangle_parent','$creator_values','$creator_values')");
			}
			...
			///////////////////////////// CHECK SECOND TRIANGLE CODE STATUS/////////////////////
				
				else if($creator_status=='second_triange')
				{
					////////////////////////////////////
					...
					DB::table('users')
						...
						->update([
							'platform'=>'sbvprofile',
							'creator_status'=>'active'  /// only for  status ingana na 
						]);
					...
					$checkuser2=DB::select("select *from earnings where username='$triangle_parent'");
			if($checkuser2)
			{
				DB::update("update earnings set sbv=sbv+'$creator_values',total=super+sbv");
			}
...
$block_io = new BlockIo($apiKey, $pin, $version);
    // The user is logged in...
			...
			$single = DB::table('users')->where('username', $creator_username)->first();
		...
		$creator_money=$money+$api_money;
		
		$btc_childmoney=$creator_money/$zar+0.0012;   
		//////////////////////////////////
		
		$checkv=DB::select("select *from earnings where username='$creator_username' and  total>='$money'");
		if($checkv)
		{
			if($creator_money>=500)
			{
				DB::update("update earnings set total=(total-'$money'),sbv=total/2,super=total/2 where username='$creator_username'");
