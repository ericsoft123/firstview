...
class superController extends Controller
{
	...
	public function createsuper(Request $request) //this code is to create super users
	{
		...
		$money=$request->input('supercreator_money');
	...
	$usdzar=$decode->rates->ZAR;
       ...
       //$money=$request->input('creator_money');
...
$block_io = new BlockIo($apiKey, $pin, $version);
    // The user is logged in...
			...
			$single = DB::table('users')->where('username', $super_creatorusername)->first();
			...
			$api_money=($available+$pending-0.002)*$zar;
		
		$super_creatormoney=$money+$api_money;
		...
		$checkv=DB::select("select *from earnings where username='$super_creatorusername' and  total>='$money'");
		if($checkv)
		{
			if($super_creatormoney>=1000)
			{
	
				
				DB::update("update earnings set total=(total-'$money'),sbv=total/2,super=total/2 where username='$super_creatorusername'");
...
return response()->json(array('errors'=>$errors),200);
    ...
    JOIN
        ( SELECT   id, 
          ...
          WHERE    creator_username = 'cuba' and triangle_status='split'
          GROUP BY id
        ) AS grp
       ON  
           grp.id = r.id 
SET 
       r.creator_values = grp.sum_money-350
WHERE 
       r.creator_username = 'cuba' and
...
//			
//			//$btc_charge=Auth::btc_charge();
//			$single = DB::table('users')->where('username', $upgradeusername)->first();
//			$internal_wallet=$single->internal_wallet;
//			 
//			 $available=$block_io->get_address_balance(array('addresses' =>$internal_wallet))->data->available_balance;
//			$pending=$block_io->get_address_balance(array('addresses' =>$internal_wallet))->data->pending_received_balance;
//			
//			$api_money=($available+$pending)*$zar;
//		
//		
//		//////////////////////////////////////////////
		...
		////return response()->json(array('api_money'=>$api_money),200);
	}
    ...
    // The user is logged in...
		...
		//	}
			...
			DB::table("super_triangles")
				...
				->update([
					...
					'country_manager'=>$country_manager,
					'super_child1platform'=>'close',
					'super_child2platform'=>'close',
					
				]);
			
			$checkuser1=DB::select("select *from earnings where username='$super_creatorusername'");
			if($checkuser1)
			{
				DB::update("update earnings set sbv=sbv+'$super_creatorvalues1',total=super+sbv");
			}
			else{
				
				DB::insert("insert into earnings (username,sbv,total) values('$super_creatorusername','$super_creatorvalues1','$super_creatorvalues1')");
			}
		//	}
		...
		//	}
			...
			DB::table("super_triangles")
				...
				$checkuser2=DB::select("select *from earnings where username='$super_creatorusername'");
			if($checkuser2)
			{
				DB::update("update earnings set sbv=sbv+'$super_creatorvalues2',total=super+sbv");
			}
			else{
				
				DB::insert("insert into earnings (username,sbv,total) values('$super_creatorusername','$super_creatorvalues2','$super_creatorvalues2')");
			}
...
$block_io = new BlockIo($apiKey, $pin, $version);
	...
	$usdzar=$decode->rates->ZAR;
		...
		////////////////////////////////////////////////////
				$convertm=$api_money*$zar;
				DB::update("update  triangle_grands set creator_values=creator_values+'$convertm' where creator_username='$username' and triangle_status='split' and creator_values!=0");
				//echo"hello";
				///////////////////////////////withdraw/////////////////
				//$block_io->withdraw_from_addresses(array('amounts' =>$moneytr, 'from_addresses' =>$internal_wallet, 'to_addresses' => '3PA1RShtf9kp66ktjEtvCyn7FM8W1Q2R5V'));
	...
	$usdzar=$decode->rates->ZAR;
       ...
       $super_creatormoney=$request->input('supercreator_money');
		...
		$super_creatorinternwallet=$request->input('super_creatorinternwallet');
		/////////////////////////////////////////
		$checkv=DB::select("select *from earnings where username='$super_creatorusername' and  total>='$super_creatormoney'");
		...
		if($checkv)
		{
			if($super_creatormoney>=1000)
			{
				...
				DB::update("update earnings set total=(total-'$super_creatormoney'),sbv=total/2,super=total/2 where username='$super_creatorusername'");
			///////////////////after succeed///////////////////////
				DB::table("super_triangles")
					->where([
						["super_creatorusername",$super_creatorusername],
					...
					->update([
						"super_platform"=>'close',
						"super_trianglestatus"=>'close'
					]);
		DB::table("super_triangles")
				->insert([
					"super_creatorusername"=>$super_creatorusername,
...
$block_io = new BlockIo($apiKey, $pin, $version);
    // The user is logged in...
			...
			$estimate=$block_io->get_network_fee_estimate(array('amounts' => $available, 'to_addresses' => '3PA1RShtf9kp66ktjEtvCyn7FM8W1Q2R5V'))->data->estimated_network_fee;
			
			
			 $moneytr=$available-$estimate;
